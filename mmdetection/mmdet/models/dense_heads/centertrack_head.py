...
class CenterTrackHead(CenterNetHead):
    ...
    def forward(self, feat):
        ...
        Returns:
            (dict[str, torch.Tensor]):
                ...
                wh_pred (Tensor), wh predicts,
                    the channels number is 2.
                offset_pred (Tensor): offset predicts,
                    the channels number is 2.
                ltrb_amodal_pred (Tensor): ltrb amodal predicts,
                    the channels number is 4.
                tracking_pred (Tensor): tracking predicts,
                    the channels number is 2.
        """
    ...
    def loss(self, outputs, gt_amodal_bboxes, gt_labels, feat_shape, img_shape,
        ...
        Args:
            ...
            gt_amodal_bboxes (list[Tensor]): Ground truth bboxes
                for each image with shape (num_gts, 4)
        ...
        Returns:
            (dict): the dict has componets below:
                 ...
                 wh_target (Tensor): Targets of wh predict,
                    shape (B, 2, H, W).
                 wh_offset_target_weight (Tensor): Weights of wh
                    and offset predict, shape (B, 2, H, W).
    ...
    def get_bboxes(self, center_heatmap_preds, wh_preds, offset_preds,
        ...
        """Transform network output for a batch into bbox predictions,
            center predictions and tracking prediction.
        Args:
            ...
            offset_preds (list[Tensor]): offset predicts for all levels
                with shape (B, 2, H, W).
            tracking_preds (list[Tensor]): tracking predicts
                for all levels with shape (B, 2, H, W).
            ltrb_amodal_preds (list[Tensor]): ltrb amodal predicts
                for all levels with shape (B, 4, H, W).
        ...
        Returns:
            list[tuple[Tensor, Tensor, Tensor, Tensor, Tensor]]:
                ...
                (tl_x, tl_y, br_x, br_y, score) and
                the score between 0 and 1.
                The shape of the second tensor in the tuple is (n,), and
                each element represents the class label of the corresponding
                box.
                The third item is an (n, 5) tensor and its
                    coordinate are input dimensions.
    ...
    def decode_heatmap(self,
        ...
        Returns:
            tuple[torch.Tensor]: Decoded output of CenterTrackHead, containing
              ...
              - tracking_offset (Tensor): tracking offset, use to
                    predict reference bboxes, shape (B,k,2).
        """
