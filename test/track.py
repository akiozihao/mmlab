...
head = CenterTrackHead(heads=dict(hm=1, reg=2, wh=2, tracking=2,
                       ...
                       weights=dict(hm=1,
                                    ...
                                    tracking=1,
                                    ltrb_amodal=0.1),
                       test_cfg=ConfigDict(topk=100,
                                           local_maximum_kernel=3,
                                           max_per_img=100),
                       train_cfg=ConfigDict(fp_disturb=0.1,
                                            lost_disturb=0.4,
                                            hm_disturb=0.05))
...
outs = [
    center_heatmap_pred, wh_pred, offset_pred, tracking_pred, ltrb_amodal_pred
]
trans = get_affine_transform(meta['c'],
                             meta['s'],
                             0, [meta['inp_width'], meta['inp_height']],
                             inv=1)
img_metas = [{
    'invert_transform': trans,
    'batch_input_shape': [meta['inp_height'], meta['inp_width']]
} for _ in center_heatmap_pred]
result_list = head.get_bboxes(*[[tensor] for tensor in outs],
                              img_metas=img_metas)
# original
...
# Track
frame_id = 0
det_bboxes, det_labels, det_bboxes_with_motion, det_bboxes_input = result_list[
    0]
bboxes, labels, ids = tracker.track(bboxes_input=det_bboxes_input,
                                    bboxes=det_bboxes,
