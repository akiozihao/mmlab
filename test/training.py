...
import_modules_from_strings([
    ...
    'mmtrack.models.mot.center_track', 'mmtrack.datasets.pipelines.transforms'
])
...
model = CTDetector(backbone=dict(
    type='DLA',
    levels=[1, 1, 1, 2, 2, 1],
    ...
    norm_cfg=dict(type='BN', momentum=0.1),
                   ...
                   bbox_head=dict(type='CenterTrackHead',
                                  heads=dict(hm=1,
                                             reg=2,
                                             wh=2,
                                             tracking=2,
...
train_loader = torch.utils.data.DataLoader(MOT(opt, 'train'),
                                           batch_size=2,
                                           shuffle=True,
                                           num_workers=4,
                                           pin_memory=True,
...
for epoch in range(1, total_epoch):
    ...
    if epoch % 5 == 0:
        torch.save(model.state_dict(),
                   f'/home/akio/dev/mmlab/models/4_epoch_{epoch}.pth')
        break
